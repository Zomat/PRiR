...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < 100000 && t > min_temp; i++) {
        ...
        if (i % 1000 == 0) {
            ...
            vector<int> all_best_orders(N * world_size);
            MPI_Allgather(best_order.data(), N, MPI_INT, all_best_orders.data(), N, MPI_INT, MPI_COMM_WORLD);

            if (global_best_cmax < best_cmax) {
                for (int j = 0; j < world_size; ++j) {
                    int offset = j * N;
                    vector<int> tempOrder(all_best_orders.begin() + offset, all_best_orders.begin() + offset + N);
                    int tempCmax = calculateCMax(p, tempOrder);
                    if (tempCmax == global_best_cmax) {
                        best_order = tempOrder;
                        best_cmax = tempCmax;
                        break;
                    }
                }
            ...
            if (world_rank == 0) {
                cout << "Synchronizacja w iteracji: " << i << ", globalny najlepszy CMAX: " << global_best_cmax << endl;
            }
        }
    }
    ...
    if (world_rank == 0) {
        for (int i = 0; i < world_size; ++i) {
            int offset = i * N;
            vector<int> g_best_order(all_best_orders.begin() + offset, all_best_orders.begin() + offset + N);
            int g_best_cmax = calculateCMax(p, g_best_order);
            if (g_best_cmax == global_best_cmax) {
                global_best_order = g_best_order;
                break;
            }
        }
